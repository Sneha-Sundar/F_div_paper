...
## Do you see differences between animal environments within just food samples?

```{r}
model_food <-  glm(is.functional ~ Generic_Host + PhG + Acq_Resist + PhG:Acq_Resist, data = test_df %>% filter(Source_Niche == "Food"), family = "binomial")

host_em <- emmeans(model_food, "Generic_Host", type = "response",infer = c(T, T))
...
```{r}
model_livestock <-  glm(is.functional ~ Generic_Host + PhG + Acq_Resist + PhG:Acq_Resist, data = test_df %>% filter(Source_Niche == "Livestock"), family = "binomial")


summary(model_livestock)

host_em <- emmeans(model_livestock, "Generic_Host",infer = c(T, T))
