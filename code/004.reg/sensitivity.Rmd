---
title: "Sensitivity analyses using a reduced dataset"
author: "Sneha Sundar"
date: "`r Sys.Date()`"
output: html_document
---

Script and statistics for Fig S9 . 


Highly similar genomes were removed and the entire logistic regression procedure was rerun. 

Highly similar genomes were identified with two criteria (see code/ANI.Rmd)
...
genomes_to_remove <- read_lines(here("data/processed/001.sample_distribution/similar_genomes_to_remove.txt"))
...
test_df <- genome_summary_df %>% 
  filter(!(target_genome %in% genomes_to_remove)) %>% 
...
## Table: Main effects model selection

```{r}
write.table(model_selection_table,paste0(plots_folder,"/","main_effects_model_selection_table_sensitive.tsv"),sep = "\t",quote = F,row.names = F)
```
...
```{r}
write.table(interaction_results, paste0(plots_folder,"/","interaction_effects_model_selection_table_sensitive.tsv"),sep = "\t",quote = F,row.names = F)
```
...
## What about differences across animal environments within each Sample source ? 

```{r}

host_em_by_sample_source <- emmeans(model_PhG_resist,pairwise ~ Generic_Host | Source_Niche, type = "response",infer = c(T, T))
host_em_by_sample_source

pairs(host_em_by_sample_source)

contrast(host_em_by_sample_source, list(`Poultry  vs. Bovine` = c(-1,1,0), 
                       `Swine  vs. Bovine` = c(-1,0,1),
                       `Swine vs. Poultry` = c(0,-1, 1)),by="Source_Niche",adjust = "sidak", options = list("infer" = c(T,T))) %>% 
  as.data.frame() %>% 
  mutate(Variable = "Animal Environment within Sample Source")

contrast(host_em_by_sample_source)
...
ggsave(paste0(plots_folder,"/","contrasts_main_pval_all_sensitive.pdf"),p_contrasts_split, units = "cm",height = 14, width = 20, dpi = 600)
```
...
ggsave(paste0(plots_folder,"/","contrasts_interactions_all_sensitive.pdf"),p_contrasts_split, units = "cm",height = 14, width = 20, dpi = 600)
