...
Script and statistics for Figure 4, S10. 
...
```{r}
poultry_prev <- binom.approx(animal.host.prev.df$Poultry * prev_result$sample_size[["Poultry"]],prev_result$sample_size[["Poultry"]]) %>% mutate(tra_gene = rownames(animal.host.prev.df),Animal_Source = "Poultry")

swine_prev <- binom.approx(animal.host.prev.df$Swine * prev_result$sample_size[["Swine"]],prev_result$sample_size[["Swine"]]) %>% mutate(tra_gene = rownames(animal.host.prev.df),Animal_Source = "Swine")

Bovine_prev <- binom.approx(animal.host.prev.df$Bovine * prev_result$sample_size[["Bovine"]],prev_result$sample_size[["Bovine"]]) %>% mutate(tra_gene = rownames(animal.host.prev.df),Animal_Source = "Bovine")
...
geom_vline(xintercept = median(animal.host.prev.df$Poultry), linetype = "dashed", linewidth = 0.5, color = animal_colours["Poultry"]) + 
geom_vline(xintercept = median(animal.host.prev.df$Bovine), linetype = "dashed", linewidth = 0.5, color = animal_colours["Bovine"]) + 
geom_vline(xintercept = median(animal.host.prev.df$Swine), linetype = "dashed", linewidth = 0.5, color = animal_colours["Swine"]) + 
